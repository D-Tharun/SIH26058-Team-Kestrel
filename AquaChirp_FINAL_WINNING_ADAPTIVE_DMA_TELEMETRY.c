...
static void compute_environment(void)
{
    ...
    /*
     * Adaptive Modulation Rules (Synced with UI decisionEngine)
     */
    if (D > 80.0f) {
        env.mod_type = 3; /* Barker-13 */
    } else if (env.res_pen > 0.7f) {
        env.mod_type = 2; /* Geometric Sweep */
    } else if (env.turbidity > 0.35f || env.res_pen > 0.4f) {
        env.mod_type = 1; /* LFM Chirp */
    } else if (D < 25.0f && env.turbidity < 0.15f) {
        env.mod_type = 0; /* CW */
    } else {
        env.mod_type = 1; /* LFM Chirp (Default) */
    }
